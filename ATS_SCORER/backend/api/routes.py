...
async def analyze_resume(
    ...
    user_id: Optional[str] = Form(None, description='Clerk user id for saving history (optional)'),
):
    warnings: List[str] = []

    import uuid

    # Handle user_id from Form or fallback
    actual_user_id = "00000000-0000-0000-0000-000000000000"
    if user_id and user_id.strip() and user_id.strip().lower() not in ("null", "undefined", ""):
        candidate_id = user_id.strip()
        try:
            uuid.UUID(candidate_id)
            actual_user_id = candidate_id
        except ValueError:
            pass
    else:
        ...
        if auth_header and auth_header.startswith("Bearer "):
            try:
                ...
                resolved_user_id = get_current_user(credentials)
                if resolved_user_id:
                    uuid.UUID(resolved_user_id)
                    actual_user_id = resolved_user_id
            except Exception as e:
                logger.warning(f"Auth header validation failed in analyze-resume: {e}")
                raise HTTPException(
                    status_code=401,
    ...
    try:
        ...
        await save_analysis(actual_user_id, filename, result)
    except Exception as exc:
