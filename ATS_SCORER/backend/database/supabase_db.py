    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 201:
                logger.error(f"Failed to save analysis: status_code={response.status_code}, response_text={response.text}")
        ...
        logger.error(f"HTTP error saving analysis to Supabase: {exc}. Response: {exc.response.text}")
        return None
