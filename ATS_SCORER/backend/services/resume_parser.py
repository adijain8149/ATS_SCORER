...
def _detect_mime_type(file_data: bytes, filename: str) -> str:
    # 1. Try python-magic if available and working
    ...
    # 2. Inspect magic bytes in header
    if file_data.startswith(b'%PDF') or b'%PDF' in file_data[:1024]:
    ...
    # 3. Fallback to extension-based MIME guess
    guessed_type, _ = mimetypes.guess_type(filename)
    if guessed_type:
        return guessed_type

    # 4. Final extension fallback if filename ends with common extensions
...
def _extract_pdf_with_pdfplumber(file_data: bytes) -> str:
    if pdfplumber is None:
        raise TextExtractionError(
            ...
            user_message='pdfplumber unavailable'
        )
    text = ''
...
def _extract_pdf_with_pypdf2(file_data: bytes) -> str:
    if PyPDF2 is None:
        raise TextExtractionError(
            ...
            user_message='PyPDF2 unavailable'
        )
    text = ''
